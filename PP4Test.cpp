...
TEST_CASE("Project 4 Test Cases for GritVM") {
  GritVM vm;
  ...
  SECTION("GritVM should be in WAITING state when no program is loaded") {
    REQUIRE(vm.run() == WAITING); //completed test!
  }

  SECTION("GritVM should throw if the file cannot be loaded and be state READY if it is") {
    CHECK_THROWS(vm.load("", initialMemory)); //completed test 
    REQUIRE(vm.load("test.gvm", initialMemory) == READY); //completed test
  }
...
TEST_CASE("Project 4 Test Cases for GritVM (EXTRA CREDIT!)") {
  GritVM vm;
  std::vector<long> initialMemory;
  std::vector<long> outputMemory;
  std::vector<long> checkMemory;
